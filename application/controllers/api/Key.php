...
class Key extends REST_Controller {
    ...
    public function index_put()
    {
        ...
        $level = $this->put('levels') ? $this->put('levels') : 1;
        $ignore_limits = ctype_digit($this->put('ignore_limits')) ? (int) $this->put('ignore_limits') : 1;

        // Insert the new key
        if ($this->_insert_key($key, ['levels' => $level, 'ignore_limits' => $ignore_limits]))
        {
            $this->response([
                'status' => TRUE,
                'keys' => $key
    ...
    public function level_post()
    {
        ...
        if ($this->_update_key($key, ['levels' => $new_level]))
        {
            $this->response([
                'status' => TRUE,
    ...
    public function suspend_post()
    {
        ...
        // Update the key level
        if ($this->_update_key($key, ['levels' => 0]))
        {
            $this->response([
                'status' => TRUE,
    ...
    public function regenerate_post()
    {
        ...
        // Insert the new key
        if ($this->_insert_key($new_key, ['levels' => $key_details->level, 'ignore_limits' => $key_details->ignore_limits]))
        {
            // Suspend old key
            $this->_update_key($old_key, ['levels' => 0]);
