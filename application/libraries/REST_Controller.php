...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        $data['Rest_Controller'] = 'rest ok!!';
        $fb1 = fopen("C:\\xampp\\htdocs\\json_data\\api_rest.json", 'w');
    ...
    public function _remap($object_called, $arguments = []) {
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE) {
            ...
            //ตอน setup ใน Class Key ... 
            //Property $methods  ... 
            //refered by put,post,get,delete on rest
            $level = isset($this->methods[$controller_method]['level']) ? $this->methods[$controller_method]['level'] : 0;
//xxxx
//print_r($level);
//echo '<br>rest->level = '.$this->rest->level .'<br>';
            // If no level is set, or it is lower than/equal to the key's level
            // the user logging on keys have levels >= settings on 
            //Class Key ... Property $methods ... must generate new keys
            //      $this->rest->level
            //ตอนส่ง put,post,get,delete เข้ามา
            $authorized = $level <= $this->rest->level;
//xxxx
//echo 'auth'. $authorized .'<br>';
            // IM TELLIN!
            if ($this->config->item('rest_enable_logging') && $log_method) {
//xxxx
//echo 'logging true <br>';
                $this->_log_request($authorized);
            }
            if ($authorized === FALSE) {
//xxxx
//echo 'authorized === false <br>';                
...
//print_r($api_key_variable);
        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));

        $this->rest->key = NULL;  /* this->rest->key */
        $this->rest->level = NULL; /* this->rest->levels */
        $this->rest->user_id = NULL;
        $this->rest->ignore_limits = FALSE;
//print_r($this->config->item('rest_database_group'));  // result is test
        // Find the key from server or arguments
        //print_r($this->input->server($key_name));
        //echo '<br>';
...
//print_r($row);
            ...
            isset($row->levels) && $this->rest->level = $row->levels;
    ...
    protected function _check_limit($controller_method) {
        ...
        //ตอนส่ง method post,put,get,delete เข้ามา
